...
def calcular_ventas(catalogo, ventas):
    ...
    for venta in ventas:
        ...
        if nombre_producto not in indice_catalogo:
            if nombre_producto not in productos_no_encontrados:
    ...
    if productos_no_encontrados:
        ...
        for producto in productos_no_encontrados:
            print(f"  - Producto '{producto}' en ventas no encontrado en catálogo")
    ...
    if productos_no_encontrados:
        print(f"  Productos no encontrados en catálogo: {len(productos_no_encontrados)}")
...
def imprimir_reporte_vtas(ventas_procesadas, tiempo_ejecucion):
    ...
    for tipo in tipos_ordenados:
        ...
        print(f"{'Total ' + tipo.capitalize() + ':':<30} {'':<12} {subtotal_tipo:>15.2f}")
        print("=" * ANCHO_LINEA)
    ...
    try:
        ...
        for tipo in tipos_ordenados:
            ...
            for producto in productos:
                ...
                archivo.write(f"{nombre:<30} {cantidad:>12.2f} {costo:>15.2f}\n")
                subtotal_tipo += costo
